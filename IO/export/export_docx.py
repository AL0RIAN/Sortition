...
class ExportRequestDOCX:
    ...
    def __print_region(self, region_name):

        for paragraph in self.__document.paragraphs:
            for run in paragraph.runs:
                if run.text == "{{REGION-NAME":
                    run.text = region_name

                if run.text == "}}":
                    run.text = ""

    def export(self, path="", file_name="noname", region_name="{{REGION-NAME}}"):

        self.__print_region(region_name)
        self.__analyze()
...
ExportRequestDOCX(data).export(file_name="Test", region_name="Запорізької області")
